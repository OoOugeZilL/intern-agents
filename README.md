...
|---|---|---|
| `main` | BIRD SQLite | Text-to-SQL agent using the BIRD benchmark dataset. **No remote server needed.** |
| `feature/mysql-support` | MySQL | Same agent + MySQL adapter. Connects to remote MySQL servers. |

### Switch between branches

```bash
# BIRD SQLite (this branch, local data only)
git checkout main
...
# Edit .env: set USEMYSQL=true and add MYSQL* variables
uv run askdata chat
```
